...
st.markdown("""Recommender systems are built using two different techniques. The first is **Content-based filtering** where recommendations are made based on the user's own preferences.
...
st.info("""You can choose between two recommendation system use cases. 
            The first uses collaborative filtering, the second uses content-based filtering to make recommendations.""")
...
if select_usecase == "Movie recommendation system 📽️":
    ...
    st.markdown("""This example showcases the use of a Recommender System for **movie recommendations** using The Movie Data Base (TMDB). <br>
